...
- Use any available thread/history, repository-search, dependency-source, or web tools only when local files are insufficient or a current reference is needed, and prefer exact source/docs over memory. If the task provides a parent thread/conversation ID, read it when you need more context than the task gives.
...
- Examine the code thoroughly, but report only the most important, actionable issues. If you found no important issues, say that directly and name the highest-risk areas you checked. Do not invent nits to justify the review.
